var echarts = echarts.init(document.getElementById('test'),'echartsTheme');
...
layui.config({
    ...
    $.ajax({
        type:"POST",
        url:"http://course.cn/User/getusercount",
        dataType:"json",
        success: function(data){
    ...
    $.ajax({
        type:"POST",
        url:"http://course.cn/User/getechartsdata",
        dataType:"json",
        success: function(msg){
            // var files = JSON.stringify(msg);
            // alert(msg.length);
            for (var i = 0 ; i <msg.length; i++) {
                var json = msg[i].major_name;
                major_name.push(json);
                setecharts(major_name);
            }
        }
    });

    function setecharts($major_name) {
        var option = {
            title: {
                    text: "各专业学生统计",
                    x: "center",
